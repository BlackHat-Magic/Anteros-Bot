...
async def assemble_conversation(channel, client):
    ...
    for message in messages:
        ...
        if (
            "Too many requests (limit 100 per hour)." in message.content
...
class ChatCog(commands.Cog):
    ...
    async def on_interaction(self, interaction):
        ...
        if not db_message:
            await interaction.response.send_message(
                "Error; Couldn't find message.", ephemeral=True
            )
            return
        ...
        if custom_id == "0":
            ...
            if not db_message:
                await interaction.followup.send(
                    "Error; message could not be found.", ephemeral=True
                )
                return
    ...
    async def on_message(self, message):
        ...
        if message.author == self.client.user or message.author.bot:
            return
        if not isinstance(message.channel, discord.Thread) and not isinstance(
        ...
        new_variant = Variant(message=new_message, text=response)
        new_request = Request(user=db_user, date=now)
            ...
            if len(paragraph) > 2000:
                ...
                for j, sentence in enumerate(sentences):
                    if len(paragraph_to_send) + len(sentence) < 2000:
                        ...
                        latest = await message.channel.send(
                            paragraph_to_send, view=None
                        )
                        paragraph_to_send = ""
                continue
    ...
    async def chat(self, interaction: discord.Interaction):
        ...
        if isinstance(interaction.channel, discord.DMChannel):
            await interaction.response.send_message(
                "You don't need to use /chat in DMs. Just send a message!\n\nTo change the system prompt, use `/system`.",
                ephemeral=True,
            )
            return
        ...
        if isinstance(interaction.channel, discord.Thread):
            await interaction.response.send_message(
                "Conversation creation not supported in threads. If I created this thread, I will respond to messages automatically. Otherwise, use `/chat` in a normal channel to start a conversation.",
                ephemeral=True,
            )
            return
        ...
        if isinstance(interaction.channel, discord.VoiceChannel):
            await interaction.response.send_message(
                "Conversations not supported in voice channels.", ephemeral=True
            )
            return
